...
{
    public partial class frmSparePartType : Form
    {
        public frmSparePartType()
        {
            InitializeComponent();
        }
        ...
        private void frmSparePartType_Load(object sender, EventArgs e)
        {
            getData();
            clear_new();
        }
        ...
        private void getData()
        {
            ...
            {
                string sql = "SELECT spp_type_id, spp_type_name FROM dbo.tbl_spare_part_type where status_id=1 order by spp_type_name";
                SqlCommand cmd = new SqlCommand(sql, conn);
                conn.Open();
                SqlDataReader reader = cmd.ExecuteReader();
                while (reader.Read())
                {
                    dgv_data.Rows.Add(reader["spp_type_id"], 0, reader["spp_type_name"], "ລົບອອກ");
                }
                reader.Close();
                set_free_item_rowNo();
            }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (master_id == 0)
                {
                    using (SqlConnection conn = new SqlConnection(globalVariable.connectionString))
                    {
                        string sql = "INSERT INTO dbo.tbl_spare_part_type (spp_type_name, status_id) VALUES (@spp_type_name, @status_id)";
                        SqlCommand cmd = new SqlCommand(sql, conn);
                        cmd.Parameters.AddWithValue("@spp_type_name", txt_name.Text);
                ...
                else
                {
                    using (SqlConnection conn = new SqlConnection(globalVariable.connectionString))
                    {
                        string sql = "UPDATE dbo.tbl_spare_part_type SET spp_type_name = @spp_type_name WHERE spp_type_id = @spp_type_id";
                        SqlCommand cmd = new SqlCommand(sql, conn);
                        cmd.Parameters.AddWithValue("@spp_type_id", master_id);
                        cmd.Parameters.AddWithValue("@spp_type_name", txt_name.Text);
                        conn.Open();
                        int rows = cmd.ExecuteNonQuery();
                        conn.Close();
        ...
        private void dgv_data_CellMouseUp(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if (columnName == "col_item_del")
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    if (get_item_id > 0)
                    {
                        string exc_qry = "update tbl_spare_part_type set status_id=2 where spp_type_id=" + get_item_id;

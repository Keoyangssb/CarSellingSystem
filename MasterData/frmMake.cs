...
namespace CarSellingSystem.MasterData
{
    public partial class frmMake : Form
    {
        ...
        private void frmMake_Load(object sender, EventArgs e)
        {
            getData();
            clear_new();
        }
        ...
        private void getData()
        {
            ...
            {
                string sql = "SELECT make_id, make_name FROM dbo.tbl_make where status_id=1 order by make_name";
                ...
                while (reader.Read())
                {
                    dgv_data.Rows.Add(reader["make_id"], 0, reader["make_name"], "ລົບອອກ");
                }
                reader.Close();
                set_free_item_rowNo();
            }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (master_id == 0)
                {
                    using (SqlConnection conn = new SqlConnection(globalVariable.connectionString))
                    {
                        string sql = "INSERT INTO dbo.tbl_make (make_name, status_id) VALUES (@make_name, @status_id)";
                        SqlCommand cmd = new SqlCommand(sql, conn);
                ...
                else
                {
                    using (SqlConnection conn = new SqlConnection(globalVariable.connectionString))
                    {
                        string sql = "UPDATE dbo.tbl_make SET make_name = @make_name WHERE make_id = @make_id";
                        SqlCommand cmd = new SqlCommand(sql, conn);
                        cmd.Parameters.AddWithValue("@make_id", master_id);
                        cmd.Parameters.AddWithValue("@make_name", txt_name.Text);
                        conn.Open();
                        int rows = cmd.ExecuteNonQuery();
                        conn.Close();
        ...
        private void dgv_data_CellMouseUp(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if (e.ColumnIndex == 3)
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    if (get_item_id > 0)
                    {
                        string exc_qry = "update tbl_make set status_id=2 where make_id=" + get_item_id;

...
namespace CarSellingSystem.MasterData
{
    public partial class frmColor : Form
    {
        ...
        private void getData()
        {
            ...
            {
                string sql = "SELECT color_id, color_name FROM dbo.tbl_color where status_id=1 order by color_name";
                ...
                while (reader.Read())
                {
                    dgv_data.Rows.Add(reader["color_id"], 0, reader["color_name"], "ລົບອອກ");
                }
                reader.Close();
                set_free_item_rowNo();
            }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (master_id == 0)
                {
                    using (SqlConnection conn = new SqlConnection(globalVariable.connectionString))
                    {
                        string sql = "INSERT INTO dbo.tbl_color (color_name, status_id) VALUES (@color_name, @status_id)";
                        SqlCommand cmd = new SqlCommand(sql, conn);
                        cmd.Parameters.AddWithValue("@color_name", txt_name.Text);
                ...
                else
                {
                    using (SqlConnection conn = new SqlConnection(globalVariable.connectionString))
                    {
                        string sql = "UPDATE dbo.tbl_color SET color_name = @color_name WHERE color_id = @color_id";
                        SqlCommand cmd = new SqlCommand(sql, conn);
                        cmd.Parameters.AddWithValue("@color_id", master_id);
                        cmd.Parameters.AddWithValue("@color_name", txt_name.Text);
                        conn.Open();
                        int rows = cmd.ExecuteNonQuery();
                        conn.Close();
        ...
        private void dgv_data_CellMouseUp(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if (columnName == "col_item_del")
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    if (get_item_id > 0)
                    {
                        string exc_qry = "update tbl_color set status_id=2 where color_id=" + get_item_id;

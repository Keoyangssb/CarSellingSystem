...
namespace CarSellingSystem.MasterData
{
    public partial class frmModel : Form
    {
        ...
        private void frmModel_Load(object sender, EventArgs e)
        {
            ...
            db.FillCombo(cboFilter, "tbl_make", "make_name", "make_id", "status_id=1", "", true);
            getData();
            clear_new();
        }
        ...
        private void getData()
        {
            ...
            {
                string sql = "SELECT * FROM v_model_make ";
                if (cboFilter.SelectedIndex > 0)
                {
                    sql = sql + " Where make_id=" + cboFilter.SelectedValue;
                }
                sql = sql + " Order by model_name";
                SqlCommand cmd = new SqlCommand(sql, conn);
                conn.Open();
                SqlDataReader reader = cmd.ExecuteReader();
                while (reader.Read())
                {
                    dgv_data.Rows.Add(reader["model_id"], 0, reader["model_name"], reader["make_id"], reader["make_name"], "ລົບອອກ");
                }
                reader.Close();
                set_free_item_rowNo();
            }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (master_id == 0)
                {
                    using (SqlConnection conn = new SqlConnection(globalVariable.connectionString))
                    {
                        string sql = "INSERT INTO dbo.tbl_model(make_id,model_name, status_id) VALUES (@make_id,@model_name, @status_id)";
                        SqlCommand cmd = new SqlCommand(sql, conn);
                        cmd.Parameters.AddWithValue("@make_id", cboMake.SelectedValue);
                ...
                else
                {
                    using (SqlConnection conn = new SqlConnection(globalVariable.connectionString))
                    {
                        string sql = "UPDATE dbo.tbl_model SET make_id=@make_id, model_name = @model_name WHERE model_id = @model_id";
                        SqlCommand cmd = new SqlCommand(sql, conn);
                        cmd.Parameters.AddWithValue("@model_id", master_id);
                        cmd.Parameters.AddWithValue("@make_id", cboMake.SelectedValue);
                        cmd.Parameters.AddWithValue("@model_name", txt_name.Text);
                        conn.Open();
                        int rows = cmd.ExecuteNonQuery();
        ...
        private void dgv_data_CellMouseUp(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if (e.ColumnIndex == 5)
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    if (get_item_id > 0)
                    {
                        string exc_qry = "update tbl_model set status_id=2 where model_id=" + get_item_id;

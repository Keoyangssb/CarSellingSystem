...
{
    public partial class frmFreeItemType : Form
    {
        public frmFreeItemType()
        {
            InitializeComponent();
        }
        int master_id = 0;
        private myClass db = new myClass();
        private void frmFreeItemType_Load(object sender, EventArgs e)
        {
            getData();
            clear_new();
        }
        private void getData()
        {
            ...
            {
                string sql = "SELECT item_type_id, item_type_name FROM dbo.tbl_free_item_type where status_id=1 order by item_type_name";
                SqlCommand cmd = new SqlCommand(sql, conn);
                conn.Open();
                SqlDataReader reader = cmd.ExecuteReader();
                while (reader.Read())
                {
                    dgv_data.Rows.Add(reader["item_type_id"], 0, reader["item_type_name"], "ລົບອອກ");
                }
                reader.Close();
                set_free_item_rowNo();
            }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (master_id == 0)
                {
                    using (SqlConnection conn = new SqlConnection(globalVariable.connectionString))
                    {
                        string sql = "INSERT INTO dbo.tbl_free_item_type (item_type_name, status_id) VALUES (@item_type_name, @status_id)";
                        SqlCommand cmd = new SqlCommand(sql, conn);
                        cmd.Parameters.AddWithValue("@item_type_name", txt_name.Text);
                ...
                else
                {
                    using (SqlConnection conn = new SqlConnection(globalVariable.connectionString))
                    {
                        string sql = "UPDATE dbo.tbl_free_item_type SET item_type_name = @item_type_name WHERE item_type_id = @item_type_id";
                        SqlCommand cmd = new SqlCommand(sql, conn);
                        cmd.Parameters.AddWithValue("@item_type_id", master_id);
                        cmd.Parameters.AddWithValue("@item_type_name", txt_name.Text);
                        conn.Open();
                        int rows = cmd.ExecuteNonQuery();
                        conn.Close();
        ...
        private void dgv_data_CellMouseUp(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if (columnName == "col_item_del")
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    if (get_item_id > 0)
                    {
                        string exc_qry = "update tbl_free_item_type set status_id=2 where item_type_id=" + get_item_id;
